...
public class ProductsController {
    ...
    public CommonResult<List<Products>> getProductsByName(
    ...
    @GetMapping("/{id}")
    public CommonResult<Products> getProductById(
        ...
        if (product != null) {
            ...
            return CommonResult.failed(ResultCode.NOT_FOUND, "商品不存在");
        }
    }
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> deleteProduct(
            @Parameter(description = "商品ID", required = true) @PathVariable Long id) {
    ...
    public CommonResult<IPage<Products>> getProductsByCategoryId(
    ...
    @GetMapping("/hot")
    public CommonResult<List<Products>> getHotProducts(
    ...
    public CommonResult<List<Products>> getRecommendProducts(
            @Parameter(description = "限制数量") @RequestParam(defaultValue = "10") Integer limit) {
    ...
    @GetMapping("/new")
    public CommonResult<List<Products>> getNewProducts(
    ...
    public CommonResult<Boolean> updateStock(
        ...
        if (result) {
            ...
            return CommonResult.failed(ResultCode.FAILED, "库存更新失败，请检查库存是否充足");
        }
    }
    ...
    public CommonResult<Boolean> batchUpdateStatus(
        ...
        if (ids == null || ids.isEmpty()) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "商品ID列表不能为空");
        }
    ...
    public CommonResult<List<Map<String, Object>>> getProductSalesRanking(
    ...
    public CommonResult<Products> getProductDetail(
            @Parameter(description = "商品ID", required = true) @PathVariable Long id) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Integer> batchImportProducts(@Valid @RequestBody List<Products> productList) {
        log.info("批量导入商品请求: count={}", productList.size());
        
        if (productList.isEmpty()) {
            log.warn("批量导入商品失败: 商品列表为空");
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "商品列表不能为空");
        }
    ...
    @GetMapping("/export")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<List<Products>> exportProducts(
