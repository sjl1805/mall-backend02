...
public class ProductReviewController {
    ...
    public CommonResult<List<ProductReview>> getReviewsByProductId(
    ...
    @GetMapping("/{id}")
    public CommonResult<ProductReview> getReviewById(
        ...
        if (review != null) {
            ...
            return CommonResult.failed(ResultCode.NOT_FOUND, "评价不存在");
        }
    }
    ...
    public CommonResult<Boolean> addProductReview(@Valid @RequestBody ProductReview productReview) {
        ...
        if (productReview.getRating() == null || productReview.getRating() < 1 || productReview.getRating() > 5) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "评分必须在1-5之间");
        }
        
        // 验证用户只能添加自己的评价
        if (!isCurrentUserOrAdmin(productReview.getUserId())) {
            log.warn("新增商品评价失败: 权限不足, userId={}", productReview.getUserId());
            return CommonResult.failed(ResultCode.FORBIDDEN, "无权为其他用户添加评价");
        }
    ...
    public CommonResult<Boolean> updateProductReview(@Valid @RequestBody ProductReview productReview) {
        ...
        if (productReview.getId() == null) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "评价ID不能为空");
        }
        ...
        if (!isCurrentUserOrAdmin(existingReview.getUserId())) {
            ...
            return CommonResult.failed(ResultCode.FORBIDDEN, "无权修改他人的评价");
        }
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("isAuthenticated()")
    public CommonResult<Boolean> deleteProductReview(
            @Parameter(description = "评价ID", required = true) @PathVariable Long id) {
        ...
        if (!isCurrentUserOrAdmin(review.getUserId())) {
            ...
            return CommonResult.failed(ResultCode.FORBIDDEN, "无权删除他人的评价");
        }
    ...
    public CommonResult<Double> calculateAverageRating(
            @Parameter(description = "商品ID", required = true) @PathVariable Long productId) {
        ...
        return CommonResult.success(averageRating);
    }
    
    @Operation(summary = "统计各评分数量", description = "获取指定商品各评分的数量分布")
    @GetMapping("/count/{productId}")
    public CommonResult<List<Map<String, Object>>> countByRating(
            @Parameter(description = "商品ID", required = true) @PathVariable Long productId) {
        ...
        return CommonResult.success(ratingCounts);
    }
    
    @Operation(summary = "根据评分范围查询评价", description = "获取指定评分范围内的商品评价")
    @GetMapping("/range/{productId}")
    public CommonResult<List<ProductReview>> selectByRatingRange(
            @Parameter(description = "商品ID", required = true) @PathVariable Long productId,
        ...
        if (minRating < 1 || minRating > 5 || maxRating < 1 || maxRating > 5 || minRating > maxRating) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "评分范围必须在1-5之间，且最低分不能高于最高分");
        }
    ...
    public CommonResult<List<ProductReview>> selectByUserId(
    ...
    public CommonResult<Boolean> batchUpdateStatus(
        ...
        if (ids == null || ids.isEmpty()) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "评价ID列表不能为空");
        }
        
        if (status < 0 || status > 2) {
            log.warn("批量更新评价状态失败: 状态值无效, status={}", status);
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "状态值必须在0-2之间");
        }
    ...
    @GetMapping("/pending")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<IPage<ProductReview>> selectPendingReviews(
            @Parameter(description = "页码") 
            @RequestParam(defaultValue = "1") int page,
            @Parameter(description = "每页数量") 
            @RequestParam(defaultValue = "10") int size) {
    ...
    public CommonResult<List<ProductReview>> selectFeaturedReviews(
            @Parameter(description = "商品ID", required = true) @PathVariable Long productId,
    ...
    public CommonResult<List<ProductReview>> selectReviewsWithImages(
            @Parameter(description = "商品ID", required = true) @PathVariable Long productId) {
    ...
    public CommonResult<Map<String, Object>> getReviewStatistics(
            @Parameter(description = "商品ID", required = true) @PathVariable Long productId) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> replyReview(
        ...
        if (replyContent == null || replyContent.trim().isEmpty()) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "回复内容不能为空");
        }
