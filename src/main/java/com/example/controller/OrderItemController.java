...
public class OrderItemController {
    ...
    public CommonResult<List<OrderItem>> getItemsByOrderId(
    ...
    public CommonResult<OrderItem> getOrderItemById(
        ...
        if (item != null) {
            ...
            return CommonResult.failed(ResultCode.NOT_FOUND, "订单商品不存在");
        }
    }
    ...
    public CommonResult<Boolean> addOrderItem(@Valid @RequestBody OrderItem orderItem) {
        ...
        if (orderItem.getQuantity() == null || orderItem.getQuantity() <= 0) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "商品数量必须大于0");
        }
    ...
    public CommonResult<Boolean> updateOrderItem(@Valid @RequestBody OrderItem orderItem) {
        ...
        if (orderItem.getId() == null) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "订单商品ID不能为空");
        }
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> deleteOrderItem(
            @Parameter(description = "订单商品ID", required = true) @PathVariable Long id) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> batchInsertOrderItems(@Valid @RequestBody List<OrderItem> orderItems) {
        log.info("批量插入订单项请求: count={}", orderItems.size());
        
        if (orderItems.isEmpty()) {
            log.warn("批量插入订单项失败: 订单项列表为空");
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "订单项列表不能为空");
        }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> deleteByOrderId(
            @Parameter(description = "订单ID", required = true) @PathVariable Long orderId) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<List<OrderItem>> getItemsByProductId(
    ...
    public CommonResult<Integer> countProductSales(
            @Parameter(description = "商品ID", required = true) @PathVariable Long productId,
    ...
    @PreAuthorize("hasRole('ADMIN') or @ordersService.selectById(#orderId)?.userId == authentication.principal.id")
    public CommonResult<List<Map<String, Object>>> getOrderItemsWithProductDetails(
            @Parameter(description = "订单ID", required = true) @PathVariable Long orderId) {
    ...
    @PreAuthorize("hasRole('ADMIN') or @orderItemService.selectById(#id)?.userId == authentication.principal.id")
    public CommonResult<Boolean> updateReviewStatus(
        ...
        if (item == null) {
            ...
            return CommonResult.failed(ResultCode.NOT_FOUND, "订单项不存在");
        }
        
        // 验证评价状态
        if (reviewStatus != 0 && reviewStatus != 1) {
            log.warn("更新订单项的评价状态失败: 评价状态无效, reviewStatus={}", reviewStatus);
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "评价状态必须为0或1");
        }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> updateRefundStatus(
            @Parameter(description = "订单项ID", required = true) @PathVariable Long id,
        ...
        if (refundStatus < 0 || refundStatus > 3) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "退款状态必须在0-3之间");
        }
    ...
    public CommonResult<List<Long>> getUserPurchasedProductIds(
    ...
    @PreAuthorize("hasRole('ADMIN') or @ordersService.selectById(#orderId)?.userId == authentication.principal.id")
    public CommonResult<Map<String, Object>> getOrderItemStatusSummary(
            @Parameter(description = "订单ID", required = true) @PathVariable Long orderId) {
