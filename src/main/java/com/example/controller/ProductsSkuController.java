...
public class ProductsSkuController {
    ...
    public CommonResult<List<ProductSku>> getSkusByProductId(
    ...
    @GetMapping("/{id}")
    public CommonResult<ProductSku> getSkuById(
        ...
        if (sku != null) {
            ...
            return CommonResult.failed(ResultCode.NOT_FOUND, "SKU不存在");
        }
    }
    ...
    public CommonResult<Boolean> addProductSku(@Valid @RequestBody ProductSku productSku) {
        ...
        if (productSku.getPrice() == null || productSku.getPrice().doubleValue() < 0) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "价格不能为空且必须大于等于0");
        }
        
        if (productSku.getStock() == null || productSku.getStock() < 0) {
            log.warn("新增SKU失败: 库存无效, stock={}", productSku.getStock());
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "库存不能为空且必须大于等于0");
        }
    ...
    public CommonResult<Boolean> updateProductSku(@Valid @RequestBody ProductSku productSku) {
        ...
        if (productSku.getId() == null) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "SKU ID不能为空");
        }
        
        if (productSku.getPrice() != null && productSku.getPrice().doubleValue() < 0) {
            log.warn("更新SKU失败: 价格无效, price={}", productSku.getPrice());
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "价格必须大于等于0");
        }
        
        if (productSku.getStock() != null && productSku.getStock() < 0) {
            log.warn("更新SKU失败: 库存无效, stock={}", productSku.getStock());
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "库存必须大于等于0");
        }
        ...
        if (result) {
            ...
            return CommonResult.failed(ResultCode.FAILED, "SKU不存在或更新失败");
        }
    }
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> deleteProductSku(
            @Parameter(description = "SKU ID", required = true) @PathVariable Long id) {
        ...
        if (result) {
            ...
            return CommonResult.failed(ResultCode.FAILED, "SKU删除失败，可能被订单或购物车引用");
        }
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> batchAddProductSkus(@Valid @RequestBody List<ProductSku> skuList) {
        log.info("批量添加SKU请求: count={}", skuList.size());
        
        if (skuList.isEmpty()) {
            log.warn("批量添加SKU失败: SKU列表为空");
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "SKU列表不能为空");
        }
        ...
        if (allSuccess) {
            ...
            return CommonResult.failed(ResultCode.FAILED, "部分SKU添加失败");
        }
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> updateSkuStock(
            @Parameter(description = "SKU ID", required = true) @PathVariable Long id,
        ...
        if (quantity < 0 && sku.getStock() < Math.abs(quantity)) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "库存不足");
        }
    ...
    @DeleteMapping("/batch")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> batchDeleteProductSkus(@RequestBody List<Long> ids) {
        log.info("批量删除SKU请求: ids={}", ids);
        
        if (ids == null || ids.isEmpty()) {
            log.warn("批量删除SKU失败: 参数无效, ids为空");
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "SKU ID列表不能为空");
        }
        ...
        if (allSuccess) {
            ...
            return CommonResult.failed(ResultCode.FAILED, "部分SKU删除失败，可能不存在或被引用");
        }
    }
} 