...
public class CategoryController {
    ...
    public CommonResult<List<Category>> getCategoriesByName(
    ...
    public CommonResult<Category> getCategoryById(
        ...
        if (category != null) {
            ...
            return CommonResult.failed(ResultCode.NOT_FOUND, "分类不存在");
        }
    }
    ...
    public CommonResult<Boolean> addCategory(@Valid @RequestBody Category category) {
        ...
        if (category.getName() == null || category.getName().trim().isEmpty()) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "分类名称不能为空");
        }
        
        if (category.getLevel() != null && (category.getLevel() < 1 || category.getLevel() > 3)) {
            log.warn("新增分类失败: 层级无效, level={}", category.getLevel());
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "分类层级必须在1-3之间");
        }
    ...
    public CommonResult<Boolean> updateCategory(@Valid @RequestBody Category category) {
        ...
        if (category.getId() == null) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "分类ID不能为空");
        }
    ...
    public CommonResult<Boolean> deleteCategory(
        ...
        if (!categoryService.checkCanDelete(id)) {
            ...
            return CommonResult.failed(ResultCode.FAILED, "无法删除，该分类下有子分类或关联商品");
        }
    ...
    public CommonResult<List<Category>> getChildrenByParentId(
    ...
    public CommonResult<List<Category>> getCategoriesByLevel(
    ...
    @GetMapping("/tree")
    public CommonResult<List<Map<String, Object>>> getCategoryTree() {
        log.info("获取分类树结构请求");
    ...
    public CommonResult<List<Map<String, Object>>> getChildrenWithProductCount(
    ...
    public CommonResult<List<Category>> getCategoryPath(
    ...
    @DeleteMapping("/batch")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> batchDeleteCategories(@RequestBody List<Long> ids) {
        log.info("批量删除分类请求: ids={}", ids);
        
        // 参数验证
        if (ids == null || ids.isEmpty()) {
            log.warn("批量删除分类失败: 参数无效, ids为空");
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "分类ID列表不能为空");
        }
        ...
        for (Long id : ids) {
            if (!categoryService.checkCanDelete(id)) {
                ...
                return CommonResult.failed(ResultCode.FAILED, "无法删除ID为" + id + "的分类，该分类下有子分类或关联商品");
            }
        }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> checkCategoryCanDelete(
            @Parameter(description = "分类ID", required = true) @PathVariable Long categoryId) {
        ...
        return CommonResult.success(canDelete);
    }
    
    @Operation(summary = "批量导入分类数据", description = "批量导入分类数据，通常用于系统初始化或数据迁移")
    @PostMapping("/batch/import")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Integer> batchImportCategories(@Valid @RequestBody List<Category> categories) {
        log.info("批量导入分类数据请求: count={}", categories.size());
        
        if (categories.isEmpty()) {
            log.warn("批量导入分类数据失败: 分类列表为空");
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "分类列表不能为空");
        }
    ...
    @GetMapping("/export")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<List<Category>> exportCategories(
    ...
    @GetMapping("/hot")
    public CommonResult<List<Map<String, Object>>> getHotCategories(
        ...
        return CommonResult.success(hotCategories);
    }
    
    @Operation(summary = "统计每个层级的分类数量", description = "统计每个层级的分类数量，用于数据分析和监控")
    @GetMapping("/count-by-level")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Map<Integer, Integer>> countCategoriesByLevel() {
    ...
    @GetMapping("/search")
    public CommonResult<List<Category>> searchCategories(
            @Parameter(description = "搜索关键词", required = true) @RequestParam String keyword) {
        log.info("根据关键词搜索分类请求: keyword={}", keyword);
        
        if (keyword == null || keyword.trim().isEmpty()) {
            log.warn("根据关键词搜索分类失败: 关键词为空");
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "搜索关键词不能为空");
        }
    ...
    public CommonResult<List<Category>> getFullCategoryTree() {
        log.info("获取完整的分类树请求");
