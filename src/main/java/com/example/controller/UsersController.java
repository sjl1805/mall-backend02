...
public class UsersController {
    ...
    public Result<Boolean> register(@Valid @RequestBody UserRegisterDTO userRegisterDTO) {
        ...
        if (result) {
            ...
            return Result.failed(ResultCode.USER_EXISTS);
        }
    }
    ...
    public Result<IPage<Users>> getUserList(
        ...
        return  Result.success(usersService.selectPage(pageParam));
    }

    @Operation(summary = "更新用户信息")
    @PutMapping("/{userId}")
    public  Result<Boolean> updateUserInfo(@PathVariable Long userId, @Valid @RequestBody UserDTO userDTO) {
    ...
    public  Result<Users> getUserByPhone(@PathVariable String phone) {
    ...
    public  Result<List<Users>> getUsersByStatus(@PathVariable Integer status) {
    ...
    @GetMapping("/search")
    public  Result<IPage<Users>> searchUsers(
    ...
    public  Result<Boolean> changePassword(
        ...
        return result ?  Result.success(true) :
                 Result.failed(ResultCode.PASSWORD_INCORRECT);
    }
    ...
    public  Result<Users> getUserBasicInfo(@PathVariable Long userId) {
    ...
    public  Result<Boolean> updateUserPhone(
        ...
        if (newPhone == null || !newPhone.matches("^1[3-9]\\d{9}$")) {
            return  Result.failed(ResultCode.VALIDATE_FAILED, "手机号格式不正确");
        }
        
        if (verifyCode == null || verifyCode.length() != 6) {
            return  Result.failed(ResultCode.VALIDATE_FAILED, "验证码格式不正确");
        }
        ...
        if (result) {
            ...
            return  Result.failed(ResultCode.FAILED, "手机号更新失败，可能已被其他用户使用");
        }
    }
} 
