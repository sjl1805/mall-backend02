...
public class UserBehaviorController {
    ...
    public CommonResult<List<UserBehavior>> getBehaviorsByUserId(
    ...
    public CommonResult<Boolean> addUserBehavior(@Valid @RequestBody UserBehavior userBehavior) {
        ...
        if (!isAdminOrSelf(userBehavior.getUserId())) {
            ...
            return CommonResult.failed(ResultCode.FORBIDDEN, "无权为其他用户添加行为记录");
        }
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> deleteUserBehavior(
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<List<UserBehavior>> getBehaviorsByType(
        ...
        if (behaviorType < 0) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "行为类型无效");
        }
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<List<UserBehavior>> getBehaviorsByUserIdAndType(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
            @Parameter(description = "行为类型", required = true) @PathVariable Integer behaviorType) {
    ...
    @GetMapping("/time")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<List<UserBehavior>> getBehaviorsByTimeRange(
    ...
    @GetMapping("/popular")
    public CommonResult<List<Products>> getPopularProducts(
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<List<Users>> getUserInterests(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
            @Parameter(description = "开始时间") 
            @RequestParam(required = false) @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") Date startTime,
            @Parameter(description = "结束时间") 
        ...
        return CommonResult.success(interests);
    }
    
    @Operation(summary = "批量插入用户行为", description = "批量添加用户行为记录，需要管理员权限")
    @PostMapping("/batch")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> batchInsertBehaviors(@Valid @RequestBody List<UserBehavior> behaviors) {
        log.info("批量插入用户行为请求: count={}", behaviors.size());
        
        if (behaviors.isEmpty()) {
            log.warn("批量插入用户行为失败: 行为列表为空");
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "行为列表不能为空");
        }
    ...
    public CommonResult<IPage<UserBehavior>> searchBehaviors(
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<Integer> countUserBehaviors(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
            @Parameter(description = "行为类型") @RequestParam(required = false) Integer behaviorType) {
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<Map<Integer, Integer>> analyzeUserBehaviorTimeDistribution(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
            @Parameter(description = "行为类型") @RequestParam(required = false) Integer behaviorType) {
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<List<Map<String, Object>>> analyzeUserBehaviorPath(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
            @Parameter(description = "开始时间") 
            @RequestParam(required = false) @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") Date startTime,
            @Parameter(description = "结束时间") 
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<BigDecimal> calculateUserActivityScore(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<Map<Integer, BigDecimal>> predictNextBehavior(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId) {
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<List<UserBehavior>> detectAbnormalBehaviors(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
        ...
        return CommonResult.success(abnormalBehaviors);
    }
    
    @Operation(summary = "分析用户行为转化", description = "计算从一种行为到另一种行为的转化率")
    @GetMapping("/conversion")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<BigDecimal> analyzeBehaviorConversion(
        ...
        if (sourceType < 0 || targetType < 0) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "行为类型必须大于或等于0");
        }
        ...
        return CommonResult.success(conversionRate);
    }
