...
public class UserAddressController {
    ...
    public CommonResult<List<UserAddress>> getAddressesByUserId(
    ...
    public CommonResult<UserAddress> getAddressById(
        ...
        if (address != null) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "收货地址不存在");
        }
    }
    ...
    public CommonResult<Boolean> addUserAddress(@Valid @RequestBody UserAddress userAddress) {
        ...
        if (!userAddressService.validateAddress(userAddress)) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "地址信息无效");
        }
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN') or @userAddressService.checkAddressOwnership(authentication.principal.id, #id)")
    public CommonResult<Boolean> deleteUserAddress(
            @Parameter(description = "收货地址ID", required = true) @PathVariable Long id) {
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<UserAddress> getDefaultAddress(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId) {
    ...
    public CommonResult<Boolean> setDefaultAddress(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
        ...
        if (!userAddressService.checkAddressOwnership(userId, addressId)) {
            ...
            return CommonResult.failed(ResultCode.FORBIDDEN, "地址不属于该用户");
        }
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<List<UserAddress>> getAddressesByRegion(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<List<UserAddress>> getFrequentlyUsedAddresses(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<UserAddress> getRecentlyUsedAddress(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId) {
    ...
    @DeleteMapping("/batch")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> batchDeleteAddresses(@RequestBody List<Long> ids) {
        log.info("批量删除收货地址请求: ids={}", ids);
        
        if (ids == null || ids.isEmpty()) {
            log.warn("批量删除收货地址失败: 参数无效, ids为空");
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "地址ID列表不能为空");
        }
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<Boolean> checkAddressOwnership(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
            @Parameter(description = "地址ID", required = true) @PathVariable Long addressId) {
        ...
        return CommonResult.success(ownership);
    }
    ...
    public CommonResult<UserAddress> copyAddress(@RequestBody Map<String, Object> params) {
        ...
        if (newAddressName == null || newAddressName.trim().isEmpty()) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "收件人姓名不能为空");
        }
        ...
        if (newAddress != null) {
            ...
            return CommonResult.success(newAddress);
        } else {
    ...
    public CommonResult<Boolean> validateAddress(@Valid @RequestBody UserAddress address) {
