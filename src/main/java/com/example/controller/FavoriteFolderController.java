...
public class FavoriteFolderController {
    ...
    public  Result<FavoriteFolder> getFavoriteFolderById(
        ...
        if (folder != null) {
            ...
            return  Result.failed(ResultCode.NOT_FOUND, "收藏夹不存在");
        }
    }
    ...
    public  Result<Boolean> updateFavoriteFolder(@Valid @RequestBody FavoriteFolder favoriteFolder) {
        ...
        if (favoriteFolder.getId() == null) {
            ...
            return  Result.failed(ResultCode.VALIDATE_FAILED, "收藏夹ID不能为空");
        }
    ...
    public  Result<List<FavoriteFolder>> getPublicFolders(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId) {
    ...
    public  Result<List<FavoriteFolder>> getOtherPublicFolders(
    ...
    public  Result<List<FavoriteFolder>> searchFoldersByName(
        ...
        if (name == null || name.trim().isEmpty()) {
            ...
            return  Result.failed(ResultCode.VALIDATE_FAILED, "搜索名称不能为空");
        }
    ...
    public  Result<Boolean> batchDelete(@RequestBody List<Long> ids) {
        ...
        if (ids == null || ids.isEmpty()) {
            ...
            return  Result.failed(ResultCode.VALIDATE_FAILED, "收藏夹ID列表不能为空");
        }
    ...
    public  Result<Boolean> batchUpdatePublicStatus(
        ...
        if (isPublic != 0 && isPublic != 1) {
            ...
            return  Result.failed(ResultCode.VALIDATE_FAILED, "公开状态必须为0或1");
        }
    ...
    public  Result<FavoriteFolder> createDefaultFolder(
        ...
        if (!existingFolders.isEmpty()) {
            ...
            return  Result.failed(ResultCode.FAILED, "用户已有收藏夹，无需创建默认收藏夹");
        }
    ...
    public Result<Boolean> moveFavorites(@RequestBody Map<String, Object> params) {
        ...
        if (favoriteIdsObj instanceof List<?>) {
            for (Object item : (List<?>) favoriteIdsObj) {
                if (item instanceof Number) {
                    favoriteIds.add(((Number) item).longValue());
                } else if (item instanceof String) {
                    try {
                        favoriteIds.add(Long.valueOf((String) item));
                    } catch (NumberFormatException e) {
                        return Result.failed(ResultCode.VALIDATE_FAILED, "收藏商品ID列表包含无效ID");
                    }
                }
            }
        } else {
            return Result.failed(ResultCode.VALIDATE_FAILED, "收藏商品ID参数格式错误");
        }
        ...
        if (folder == null) {
            ...
            return  Result.failed(ResultCode.NOT_FOUND, "目标收藏夹不存在");
        }
