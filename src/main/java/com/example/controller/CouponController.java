...
public class CouponController {
    ...
    public CommonResult<List<Coupon>> getCouponsByName(
    ...
    @GetMapping("/{id}")
    public CommonResult<Coupon> getCouponById(
        ...
        if (coupon != null) {
            ...
            return CommonResult.failed(ResultCode.NOT_FOUND, "优惠券不存在");
        }
    }
    ...
    public CommonResult<Boolean> addCoupon(@Valid @RequestBody Coupon coupon) {
        ...
        if (coupon.getValue() != null && coupon.getValue().compareTo(BigDecimal.ZERO) < 0) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "优惠券面值不能为负数");
        }
        
        if (coupon.getNum() != null && coupon.getNum() <= 0) {
            log.warn("新增优惠券失败: 数量无效, num={}", coupon.getNum());
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "优惠券数量必须大于0");
        }
    ...
    public CommonResult<Boolean> updateCoupon(@Valid @RequestBody Coupon coupon) {
        ...
        if (coupon.getId() == null) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "优惠券ID不能为空");
        }
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Boolean> deleteCoupon(
            @Parameter(description = "优惠券ID", required = true) @PathVariable Long id) {
    ...
    public CommonResult<Boolean> setCouponValidity(
        ...
        try {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "日期格式错误，请使用yyyy-MM-dd HH:mm:ss格式");
        }
    }
    ...
    public CommonResult<Boolean> setCouponConditions(
        ...
        if (minAmount.compareTo(BigDecimal.ZERO) < 0) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "最低使用金额不能为负数");
        }
    ...
    public CommonResult<List<Coupon>> getAvailableCoupons(
        ...
        if (amount.compareTo(BigDecimal.ZERO) < 0) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "订单金额不能为负数");
        }
    ...
    @GetMapping("/expiring")
    public CommonResult<List<Coupon>> getExpiringCoupons(
        ...
        if (days <= 0) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "天数必须大于0");
        }
    ...
    public CommonResult<Boolean> decreaseCouponNum(
            @Parameter(description = "优惠券ID", required = true) @PathVariable Long id,
        ...
        if (count <= 0) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "减少数量必须大于0");
        }
        ...
        if (coupon.getNum() < count) {
            ...
            return CommonResult.failed(ResultCode.FAILED, "优惠券数量不足");
        }
    ...
    @GetMapping("/{id}/check")
    public CommonResult<Coupon> checkCouponAvailable(
            @Parameter(description = "优惠券ID", required = true) @PathVariable Long id,
        ...
        if (coupon != null) {
            ...
            return CommonResult.failed(ResultCode.FAILED, "优惠券不可用于当前订单金额");
        }
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<List<Map<String, Object>>> getCouponStatistics(
        ...
        if (startDate.after(endDate)) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "开始日期不能晚于结束日期");
        }
    ...
    @GetMapping("/popular")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<List<Map<String, Object>>> getPopularCoupons(
        ...
        if (limit <= 0) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "限制数量必须大于0");
        }
        ...
        return CommonResult.success(popularCoupons);
    }
    ...
    public CommonResult<Boolean> batchUpdateStatus(
        ...
        if (ids == null || ids.isEmpty()) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "优惠券ID列表不能为空");
        }
        
        if (status != 0 && status != 1) {
            log.warn("批量更新优惠券状态失败: 状态无效, status={}", status);
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "状态必须为0或1");
        }
    ...
    public CommonResult<List<Coupon>> getNotReceivedCoupons(
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<List<Coupon>> advancedSearch(@RequestBody Map<String, Object> params) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Map<String, Object>> calculateCouponEffectiveness(
        ...
        return CommonResult.success(effectiveness);
    }
    
    @Operation(summary = "自动失效过期优惠券", description = "将已过期但状态仍为有效的优惠券状态更新为失效")
    @PostMapping("/expireOutdated")
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Integer> expireOutdatedCoupons() {
    ...
    public CommonResult<List<Coupon>> recommendCouponsForUser(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
        ...
        if (limit <= 0) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "推荐数量必须大于0");
        }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public CommonResult<Integer> batchIssueCoupons(@RequestBody Map<String, Object> params) {
        ...
        if (userIds.isEmpty()) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "用户ID列表不能为空");
        }
        ...
        if (coupon.getNum() < userIds.size()) {
            ...
            return CommonResult.failed(ResultCode.FAILED, "优惠券数量不足，无法满足所有用户发放需求");
        }
        ...
        if (successCount == 0) {
            return CommonResult.failed(ResultCode.FAILED, "发放失败，可能是优惠券已失效或数量不足");
        } else if (successCount < userIds.size()) {
            return CommonResult.success(successCount, "部分用户发放失败，可能已领取过该优惠券");
        } else {
