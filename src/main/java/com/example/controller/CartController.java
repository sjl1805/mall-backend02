...
public class CartController {
    ...
    public CommonResult<List<Cart>> getCartsByUserId(
    ...
    public CommonResult<Cart> getCartById(
        ...
        if (cart != null) {
            ...
            return CommonResult.failed(ResultCode.NOT_FOUND, "购物车记录不存在");
        }
    }
    ...
    public CommonResult<Boolean> addCart(@Valid @RequestBody Cart cart) {
        ...
        if (cart.getProductId() == null) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "商品ID不能为空");
        }
    ...
    public CommonResult<Boolean> updateCart(@Valid @RequestBody Cart cart) {
        ...
        if (cart.getId() == null) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "购物车ID不能为空");
        }
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN') or @cartService.selectById(#id)?.userId == authentication.principal.id")
    public CommonResult<Boolean> deleteCart(
            @Parameter(description = "购物车ID", required = true) @PathVariable Long id) {
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<Cart> getCartByUserIdAndProductId(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
        ...
        if (cart != null) {
            ...
            return CommonResult.failed(ResultCode.NOT_FOUND, "购物车中未找到该商品");
        }
    }
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<List<Map<String, Object>>> getCartDetail(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId) {
    ...
    @PreAuthorize("hasRole('ADMIN') or @cartService.selectById(#id)?.userId == authentication.principal.id")
    public CommonResult<Boolean> updateChecked(
            @Parameter(description = "购物车ID", required = true) @PathVariable Long id,
        ...
        if (checked != 0 && checked != 1) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "选中状态必须为0或1");
        }
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<Boolean> updateAllChecked(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
            @Parameter(description = "选中状态: 0-未选中 1-已选中", required = true) @PathVariable Integer checked) {
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<List<Cart>> getCheckedCartsByUserId(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId) {
        ...
        return CommonResult.success(checkedCarts);
    }
    
    @Operation(summary = "获取选中商品详情", description = "获取已选中商品的详细信息，用于订单确认")
    @GetMapping("/checked-detail/{userId}")
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<List<Map<String, Object>>> getCheckedCartDetail(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId) {
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<Boolean> deleteChecked(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId) {
    ...
    public CommonResult<Boolean> batchDelete(@RequestBody List<Long> ids) {
        ...
        if (ids == null || ids.isEmpty()) {
            ...
            return CommonResult.failed(ResultCode.VALIDATE_FAILED, "购物车ID列表不能为空");
        }
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<Integer> countByUserId(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId) {
    ...
    public CommonResult<Boolean> addOrUpdateCart(
    ...
    @PreAuthorize("hasRole('ADMIN') or authentication.principal.id == #userId")
    public CommonResult<BigDecimal> calculateCheckedAmount(
            @Parameter(description = "用户ID", required = true) @PathVariable Long userId) {
