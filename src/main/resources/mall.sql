...
ALTER TABLE category ADD INDEX idx_level_sort (level, sort);
...
ALTER TABLE coupon ADD INDEX idx_type (type);
...
ALTER TABLE user_address ADD INDEX idx_phone (receiver_phone);
...
ALTER TABLE favorite_folder ADD INDEX idx_sort (sort);
...
ALTER TABLE order_item ADD INDEX idx_product_sales (product_id);
...
ALTER TABLE user_coupon ADD INDEX idx_coupon_valid (coupon_id, status, get_time);
...
ALTER TABLE product_spec ADD INDEX idx_spec_values (spec_values(64));
...
ALTER TABLE product_sku ADD INDEX idx_price_stock (price, stock);
...
ALTER TABLE product_review ADD INDEX idx_user_order (user_id, order_id);
...
ALTER TABLE product_favorite ADD INDEX idx_folder (folder_id);
...
ALTER TABLE recommend_product ADD INDEX idx_algorithm (algorithm_version);
...
VALUES ('user', '$2a$10$k6n948le92ccts2LWj4mrubCqe4XaS3Yi8K6EiiBmAuMIy5m/B5zq', '张三', '13800000001', 'user1@mall.com',
        1, 1),
       ('user2', '$2a$10$k6n948le92ccts2LWj4mrubCqe4XaS3Yi8K6EiiBmAuMIy5m/B5zq', '李四', '13800000002',
        'user2@mall.com', 2, 1),
       ('user3', '$2a$10$k6n948le92ccts2LWj4mrubCqe4XaS3Yi8K6EiiBmAuMIy5m/B5zq', '王五', '13800000003',
        'user3@mall.com', 1, 2),
       ('admin', '$2a$10$k6n948le92ccts2LWj4mrubCqe4XaS3Yi8K6EiiBmAuMIy5m/B5zq', '管理员', '13800000004',
        'admin@mall.com', 0, 9);

-- 2. 商品分类表（10条）
INSERT INTO category (parent_id, NAME, LEVEL, sort)
VALUES (NULL, '手机数码', 1, 1),
       (1, '智能手机', 2, 1),
       (2, '5G手机', 3, 1),
       (NULL, '电脑办公', 1, 2),
       (4, '笔记本', 2, 1),
...
VALUES (3, '华为Mate50', '5G旗舰手机', 5999.00, 100, 1, '[
  "/images/backend.png"
]'),
...
VALUES ('新人满1000减200', 1, 200.00, 1000.00, '2023-01-01', '2024-12-31'),
       ('大家电9折券', 2, 0.90, 3000.00, '2023-01-01', '2024-12-31'),
       ('手机专用券', 1, 500.00, 4000.00, '2023-01-01', '2024-12-31'),
       ('全品类8折', 2, 0.80, 2000.00, '2023-06-01', '2023-12-31'),
       ('暑促优惠券', 1, 300.00, 1500.00, '2023-07-01', '2023-08-31'),
       ('冰箱专属券', 1, 800.00, 5000.00, '2023-01-01', '2024-12-31'),
       ('笔记本立减', 1, 1000.00, 8000.00, '2023-09-01', '2023-12-31'),
       ('空调折扣券', 2, 0.85, 3000.00, '2023-01-01', '2024-12-31'),
       ('年货节通用券', 1, 150.00, 1000.00, '2023-12-01', '2024-02-10'),
       ('会员专属券', 2, 0.75, 5000.00, '2023-01-01', '2024-12-31');

-- 5. 用户地址表（10条）
INSERT INTO user_address (user_id, is_default, receiver_name, receiver_phone, province, city, district, detail_address)
VALUES (1, 1, '张三', '13800000001', '北京市', '北京市', '海淀区', '中关村大街1号'),
       ...
       (1, 0, '张三仓库', '13800000001', '广东省', '广州市', '天河区', '珠江新城');

-- 6. 订单表（10条）
INSERT INTO orders (user_id, order_no, total_amount, pay_amount, STATUS, receiver_name, receiver_phone,
                    receiver_address)
VALUES (1, '202309010001', 11998.00, 11798.00, 3, '张三', '13800000001', '北京市海淀区中关村大街1号'),
       ...
       (1, '202309010010', 5999.00, 5999.00, 4, '张三', '13800000001', '广州市天河区珠江新城');

-- 7. 订单项表（10条）
INSERT INTO order_item (order_id, product_id, product_name, product_image, price, quantity, total_amount)
VALUES (1, 1, '华为Mate50', '/images/backend.png', 5999.00, 2, 11998.00),
       ...
       (10, 6, '西门子冰箱', '/images/backend.png', 5999.00, 1, 5999.00);

-- 8. 收藏夹表（10条）
INSERT INTO favorite_folder (user_id, NAME, description, is_public)
VALUES (1, '电子产品', '手机电脑收藏', 1),
       (1, '家电清单', '家用电器收藏', 0),
       (2, '数码好物', '最新科技产品', 1),
       (2, '办公设备', '办公用品收藏', 0),
       (3, '游戏装备', '电竞设备合集', 1),
...
VALUES (1, '{"颜色":"黑色","存储":"128GB"}', 5999.00, 50),
...
VALUES (1, 1, 1, 1, '2025-01-01', '2025-12-31', 'v2.1'),
       ...
       (10, 2, 10, 1, '2025-08-01', '2025-01-31', 'v2.1');
